namespace HeCopUI_Framework.SuperNotifyIcon.Finder
{
    using System;
            ...
            for (int i = 0; !found && i < natoolbarwindows.Count; i++)
            {
                ...
                try
                {
                    ...
                    for (int j = 0; !found && j < buttoncount; j++)
                    {
                        ...
                        uint niidnew;
                        NativeMethods.ReadProcessMemory(naprocesshandle, new IntPtr(niinfopointer.ToInt64() + Marshal.SizeOf(typeof(IntPtr))), out niidnew, Marshal.SizeOf(typeof(uint)), out bytesread);
